...
async def get_service_command(message: Message, session: AsyncSession):
    service_repo = ServiceRepository(session)
    data = await service_repo.get_all()
...
async def get_service_answer_process(message: Message, session: AsyncSession):
    service_repo = ServiceRepository(session)
...
async def add_service_command(message: Message, state: FSMContext):
...
async def fill_service_answer_process(message: Message, state: FSMContext):
...
async def add_service_to_db_process(message: Message, state: FSMContext, session: AsyncSession):
...
async def choose_service_for_delete_command(message: Message, session: AsyncSession, state: FSMContext):
    service_repo = ServiceRepository(session)
    data = await service_repo.get_all()
    await state.set_state(FSMFillServiceForm.choose_service)
    await message.answer(SERVICES_LEXICON['choose_service_for_delete'], reply_markup=kb_builder(data=data,
                                                                                                cancel_btn=True))


@router.message(StateFilter(FSMFillServiceForm.choose_service))
async def delete_service_process(message: Message, session: AsyncSession):
    service_repo = ServiceRepository(session)
    await service_repo.delete(service_name=message.text)
    data = await service_repo.get_all()
